...
<script>
    ...
    export default {
        ...
        created() {
            //请求数据
            axios.get("http://localhost:9378/api/getSingerData/").then(data=>{
                // eslint-disable-next-line no-console
                console.log(data.data);
                this.singerlist=data.data;   //这里获取的是歌手列表
            })
        },
        methods:{
            ...
            selectSinger(singerItem){//由子组件的点击事件触发的父组件事件 listview绑定了事件selectSinger
                this.$router.push({
