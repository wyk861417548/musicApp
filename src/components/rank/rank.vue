...
<script >
    ...
    export default {
        ...
        created() {
            axios.get("http://localhost:9378/api/getRankData").then((data)=>{
        ...
        methods: {
            ...
            selectItem(item) {
                ...
                axios.get(`http://localhost:9378/api/getRankDetail/${item.topId}`).then((data)=>{
                    // eslint-disable-next-line no-console
                    console.log(data.data)
                    // this.$router.push({
                    ...
                    // });
                    console.log(item.topId);
                    this.$router.push(`/rank/${item.topId}`);//更改路由
