...
<script>
    ...
    export default {
        ...
        beforeCreate() {
            axios.get("http://localhost:9378/api/getRecommend").then((data) => {
                console.log(data);
                this.sliderRecommends = data.data.pop().categoryList;//最后一个歌单列表作为轮播图的部分
        ...
        methods:{
            ...
            selectItem(item){
                axios.get(`http://localhost:9378/api/recommendDetail/${item.id}`)
                    .then(data=>{
                    this.$router.push(`/recommend/${item.id}`);//更改路由
                    //设置vuex
